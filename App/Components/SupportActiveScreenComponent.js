...
export default function SupportActiveScreenComponent() {
  ...
  const CardItem = ({item}) => {
    ...
    if (item.status == 'Open' || item.status == 'Customer-Reply') {
      return (
        <TouchableOpacity
          onPress={() =>
            navigation.navigate('SupportSubScreen', {ticketid: item?.ticketid})
          }
          style={styles.card}>
